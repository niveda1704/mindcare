...
const Screening = () => {
    ...
    const questionnaires = {
        'PHQ-9': {
            name: t('screening.phq9.name'),
            shortName: t('screening.phq9.shortName'),
            description: t('screening.phq9.description'),
            questions: t('screening.phq9.questions', { returnObjects: true }),
            options: t('screening.options', { returnObjects: true }),
        },
        'GAD-7': {
            name: t('screening.gad7.name'),
            shortName: t('screening.gad7.shortName'),
            description: t('screening.gad7.description'),
            questions: t('screening.gad7.questions', { returnObjects: true }),
            options: t('screening.options', { returnObjects: true }),
        },
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setResult(response.data.result);
            toast.success(t('screening.successMsg'));
        } catch (error) {
            console.error('Submission failed', error);
            const msg = error.response?.data?.message || t('screening.errorMsg');
            toast.error(msg);
        } finally {
            setLoading(false);
        }
        ...
        return (
            <div className="max-w-2xl mx-auto py-12 px-6 text-center space-y-8">
                ...
                >
                    ...
                    <h2 className="text-3xl font-bold text-gray-800 mb-2 tracking-tight">{t('screening.results.title')}</h2>
                    <p className="text-sm font-bold text-gray-400 uppercase tracking-[0.3em] mb-8">{t('screening.results.subtitle')}</p>
                    ...
                    <div className={`p-8 rounded-[2rem] mb-10 text-left border ${result.riskLevel === 'high' ? 'bg-status-danger/5 border-status-danger/10 text-status-danger' : 'bg-status-success/5 border-status-success/10 text-status-success'}`}>
                        <div className="flex justify-between items-center mb-4">
                            <h4 className="font-black uppercase tracking-widest text-[10px]">{t('screening.results.profile')}</h4>
                            <span className="text-2xl font-black">{result.riskLevel.toUpperCase()}</span>
                        </div>
                        <p className="text-[13px] font-medium leading-relaxed">
                            {result.riskLevel === 'high'
                                ? t('screening.results.highRiskMsg')
                                : t('screening.results.lowRiskMsg')}
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        ...
                        {result.riskLevel === 'high' && (
                            ...
                            >
                                {t('screening.connectGuide')} <ArrowRight size={14} className="ml-2 inline" />
                            </motion.button>
                        )}
                    </div>
                </motion.div>
                ...
                <>
                    <header className="relative py-12 text-center">
                        ...
                        >
                            <span className="inline-block px-4 py-1.5 rounded-full bg-morning-accent-lavender/10 text-morning-accent-lavender text-[10px] font-bold uppercase tracking-[0.2em] mb-4">
                                {t('screening.headerTag')}
                            </span>
                            <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4 tracking-tight">{t('screening.headerTitle')} <span className="text-morning-accent-lavender">{t('screening.headerTitleHighlight')}</span></h1>
                            <p className="text-lg text-gray-500 max-w-2xl mx-auto font-medium leading-relaxed">
                                {t('screening.headerDesc')}
                            </p>
                        </motion.div>
                    </header>
                        ...
                        {Object.entries(questionnaires).map(([key, q], idx) => (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] text-gray-800 group-hover:translate-x-2 transition-transform duration-700">
                                    {t('screening.beginScan')} <div className="w-8 h-8 rounded-full bg-gray-800 text-white flex items-center justify-center shrink-0 ml-1 group-hover:bg-morning-accent-lavender transition-colors duration-700"><ChevronRight size={14} /></div>
                                </div>
                            </motion.div>
                        ))}
                    </div>
                    ...
                    >
                        <div className="flex items-start gap-4">
                            ...
                            <div className="pt-1">
                                <p className="text-xs font-bold text-gray-800 uppercase tracking-widest mb-2">{t('screening.privacyTitle')}</p>
                                <p className="text-[12px] text-gray-500 font-medium leading-relaxed">
                                    {t('screening.privacyDesc')}
                                </p>
                            </div>
                        </div>
                    </motion.div>
                    ...
                    >
                            ...
                            <div className="flex justify-between items-start mb-10">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.3em] flex items-center gap-2">
                                        {t('screening.inProgress')} <Sparkles size={10} className="text-morning-accent-lavender animate-pulse" />
                                    </p>
                                </div>
                                <button
                                    onClick={() => setSelectedType(null)}
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-end">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-800">
                                        {t('screening.question')} {currentQuestion + 1}
                                    </span>
                                    <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest">
                                        {Math.round(((currentQuestion + 1) / questionnaires[selectedType]?.questions.length) * 100)}% {t('screening.complete')}
                                    </span>
                                </div>
                        ...
                        <div className="p-10 space-y-10">
                            ...
                            <div className="flex justify-between items-center pt-8 border-t border-gray-50">
                                ...
                                >
                                    <ChevronLeft size={16} /> {t('screening.previous')}
                                </button>
                                ...
                                {currentQuestion === questionnaires[selectedType].questions.length - 1 && answers[currentQuestion] !== null ? (
                                    ...
                                    >
                                        {loading ? <Loader className="animate-spin" size={18} /> : t('screening.finalize')}
                                    </motion.button>
                                ) : (
                                    <span className="text-[10px] text-gray-300 font-black tracking-[0.3em] uppercase">{t('screening.reflectSelect')}</span>
                                )}
                            </div>
                        </div>
                    </motion.div>
